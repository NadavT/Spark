...
bool VulkanRenderer::begin()
{
    ...
    result = vkAcquireNextImageKHR(m_context.m_device, m_context.m_swapChain, UINT64_MAX,
                                   m_semaphores[m_currentFrame][0], VK_NULL_HANDLE, &m_currentImageIndex);
...
VulkanFramebuffer *VulkanRenderer::createFramebuffer(VulkanFramebufferType type, bool clear, bool resolve)
{
    ...
    case VulkanFramebufferType::Type2D:
        if (m_context.m_msaaSamples != VK_SAMPLE_COUNT_1_BIT)
        {
            m_framebuffers.push_back(
                std::make_unique<VulkanFramebuffer2D>(m_context, m_multisampleImageView, clear, resolve));
        }
        else
        {
            m_framebuffers.push_back(
                std::make_unique<VulkanFramebuffer2D>(m_context, (VkImageView)VK_NULL_HANDLE, clear, resolve));
        }
    ...
    case VulkanFramebufferType::Type3D:
        ...
        {
            m_framebuffers.push_back(
                std::make_unique<VulkanFramebuffer3D>(m_context, m_multisampleImageView, clear, resolve));
        }
        else
        {
            m_framebuffers.push_back(
                std::make_unique<VulkanFramebuffer3D>(m_context, (VkImageView)VK_NULL_HANDLE, clear, resolve));
        }
        return m_framebuffers.back().get();
    default:
...
{
    auto found_it = std::find_if(m_framebuffers.begin(), m_framebuffers.end(),
                                 [&](std::unique_ptr<VulkanFramebuffer> &p) { return p.get() == framebuffer; });
...
VulkanPipeline *VulkanRenderer::createPipeline(VulkanPipelineType type, VulkanFramebuffer &framebuffer)
{
    ...
    case VulkanPipelineType::Type3D:
        ...
        return m_pipelines.back().get();
    case VulkanPipelineType::Type3DLights:
        m_pipelines.push_back(std::make_unique<VulkanPipeline3DLights>(m_context, framebuffer));
        return m_pipelines.back().get();
    case VulkanPipelineType::Type3DModel:
        m_pipelines.push_back(std::make_unique<VulkanPipeline3DModel>(m_context, framebuffer));
        return m_pipelines.back().get();
...
{
    auto found_it = std::find_if(m_pipelines.begin(), m_pipelines.end(),
                                 [&](std::unique_ptr<VulkanPipeline> &p) { return p.get() == pipeline; });
...
void VulkanRenderer::beginRenderPass(VkCommandBuffer commandBuffer, VkRenderPass renderPass, VkFramebuffer framebuffer,
                                     uint32_t clearValueCount, const VkClearValue *clearValues)
{
...
void VulkanRenderer::createUniformBuffers(VkDeviceSize size, std::vector<VkBuffer> &uniformBuffers,
...
void VulkanRenderer::createUniformBuffers(VkDeviceSize size, std::vector<std::vector<VkBuffer>> &uniformBuffers,
...
void VulkanRenderer::createMultisamplesResources()
{
    ...
    m_context.createImage(m_context.m_swapChainExtent.width, m_context.m_swapChainExtent.height, 1,
                          m_context.m_msaaSamples, colorFormat, VK_IMAGE_TILING_OPTIMAL,
...
void VulkanRenderer::fillResolveCommandBuffers()
{
    for (int i = 0; i < m_resolveCommandBuffers.size(); i++)
    {
        ...
        beginRenderPass(m_resolveCommandBuffers[i], m_resolveFramebuffer->getRenderPass(),
                        m_resolveFramebuffer->getswapChainFramebuffers()[i]);
