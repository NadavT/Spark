...
namespace Spark
{
	...
	void VulkanLayerRenderer3D::OnAttach()
	{
		...
		m_renderer.createUniformBuffers(sizeof(Transformation3D), m_uniformTransformations, m_uniformTransformationsMemory, (unsigned int)m_drawables.size());
	...
	void VulkanLayerRenderer3D::addDrawable(std::shared_ptr<Drawable>& drawable) 
	{
		...
		if (m_isAttached)
		{
			...
			{
				m_renderer.addUniformBuffers(sizeof(Transformation3D), m_uniformTransformations, m_uniformTransformationsMemory);
			...
			if (m_textureDescriptorOffset.find(cube->getTexture().getName()) == m_textureDescriptorOffset.end())
			{
				...
				m_textureDescriptorOffset[cube->getTexture().getName()] = static_cast<unsigned int>(m_textureDescriptorSets.size());
	...
	void VulkanLayerRenderer3D::createCommandBuffers()
	{
		...
		for (VkCommandBuffer commandBuffer : m_commandBuffers) {
			...
			for (size_t j = 0; j < m_drawables.size(); j++)
			{
				...
				m_pipeline->bind(commandBuffer, m_transfomationDescriptorSets[j][i], m_textureDescriptorSets[m_textureDescriptorOffset[cube->getTexture().getName()]][i]);
