...
namespace Spark
{
    ...
    {
#ifdef SPARK_PLATFORM_VULKAN
        SPARK_CORE_ASSERT(ResourceManager::getResource(name) == nullptr, "A resource with the name \"" + name + "\" is already loaded");
        ...
        SPARK_CORE_ASSERT(ResourceManager::getResource("SPARK_CORE:VulkanImage:" + name) == nullptr, "A resource with the name \"" + name + "\" is already loaded");
        ResourceManager::loadResource(std::make_unique<VulkanTextureImage>(context, "SPARK_CORE:VulkanImage:" + name, path));

        VulkanTextureImage* texture_image = reinterpret_cast<VulkanTextureImage*>(ResourceManager::getResource("SPARK_CORE:VulkanImage:" + name));
        VulkanTextureSampler* sampler = reinterpret_cast<VulkanTextureSampler*>(ResourceManager::getResource("SPARK_CORE:VulkanImageSampler"));
